...
module.exports = {
    removeHCP: function(){
        return Q.try(function() {
            var command = ['meteor', 'remove', 'ddp', 'hot-code-push'];
            return execute(command, 'remove hot code push packages');
        });
    },
    addHCP: function(){
        return Q.try(function() {
            var command = ['meteor', 'add', 'ddp', 'hot-code-push'];
            return execute(command, 'add hot code push packages');
        });
    },
    ...
    addIndexFile: function(program) {
        return Q.try(function(resolve, reject) {
            ...
            const bodyPath = path.join(buildPath, 'body.html')
            const body = fs.readFileSync(bodyPath, {encoding: 'utf8'});
